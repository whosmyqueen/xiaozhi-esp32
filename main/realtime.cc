...
#define TAG "RtcConfig"

Realtime::Realtime() {}

Realtime::~Realtime() {}

void Realtime::SetCheckVersionUrl(std::string check_version_url) { check_version_url_ = check_version_url; }
...
void Realtime::Upgrade(const std::string& firmware_url) {
	...
	while (true) {
		...
		if (!image_header_checked) {
			...
			if (image_header.size() >= sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t) + sizeof(esp_app_desc_t)) {
				...
				memcpy(&new_app_info, image_header.data() + sizeof(esp_image_header_t) + sizeof(esp_image_segment_header_t),
...
void Realtime::StartUpgrade(std::function<void(int progress, size_t speed)> callback) {
	upgrade_callback_ = callback;
	Upgrade(firmware_url_);
}

std::vector<int> Realtime::ParseVersion(const std::string& version) {
...
bool Realtime::IsNewVersionAvailable(const std::string& currentVersion, const std::string& newVersion) {
